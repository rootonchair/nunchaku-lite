...
def svdq_quantize_w4a4_act_fuse_lora_cuda(
    ...
    Args:
        ...
        w8a8: Whether to use the int8 (unpacked, byte-per-element) activation
            layout instead of the default packed int4 layout.
    ...
    if output is None:
        if w8a8:
            output = torch.empty(batch_size_pad, channels, dtype=torch.uint8, device=input.device)
        else:
    ...
    if oscales is None:
        if fp4:
            ...
            if channels % 32 != 0:
                raise ValueError("INT8 activation channels must be divisible by 32")
            oscales = torch.empty(channels // 32, batch_size_pad, dtype=input.dtype, device=input.device)
        else:
            if channels % 64 != 0:
    ...
    get_ops().quantize_w4a4_act_fuse_lora(input, output, oscales, lora_down, lora_act_out, smooth, fuse_glu, fp4, w8a8)
